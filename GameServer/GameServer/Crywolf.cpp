...
int CCrywolf::GetLeftTimeForEventSec()
{
	// Ora + minutul evenimentului
	int startHour = this->m_StartTimeInfo[0].m_Hour;
	int startMinute = this->m_StartTimeInfo[0].m_Minute;

	// Timpul curent (secunde)
	time_t now = time(0);

	// Descompunem timpul în struct tm
	tm* localTime = localtime(&now);

	// Construim timpul startului pentru AZI
	tm eventTime = *localTime;
	eventTime.tm_hour = startHour;
	eventTime.tm_min = startMinute;
	eventTime.tm_sec = 0;

	// Convertim în time_t
	time_t eventTimestamp = mktime(&eventTime);

	// Dacă ora evenimentului azi a trecut → mutăm pentru mâine
	if (eventTimestamp <= now)
	{
		eventTimestamp += 24 * 60 * 60; // +1 zi
	}

	int remainSec = (int)difftime(eventTimestamp, now);
