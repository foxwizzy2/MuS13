...
void CObjectManager::CharacterGameCloseSet(int aIndex, int type) // OK
{
	...
	if (lpObj->Connected == OBJECT_ONLINE)
	{
		...
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Don’t run, man… hold on there.");
			return;
		}
...
bool CObjectManager::CharacterLevelUpPointAdd(LPOBJ lpObj, int type, int amount, bool send) // OK
{
	...
	{
		GCMessagePopupSend(lpObj, "You can’t change your points during MUFC.");
		return 0;
	}
...
bool CObjectManager::CharacterUseJewelOfBles(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	...
	{
		GCMessagePopupSend(lpObj, "A straight dick cannot be repaired. :(");
		return 0;
	}
...
bool CObjectManager::CharacterUseCustomJewel(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	...
	CItem* lpItem = &lpObj->Inventory[TargetSlot];
	// ===== S12 Custom Jewel System =====
	if (gCustomJewelSystem.IsCustomJewel(lpObj->Inventory[SourceSlot].m_Index))
	{
		if (gCustomJewelSystem.ProcessCustomJewel(lpObj, SourceSlot, TargetSlot))
		{
			return 1;
		}
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "This jewel cannot be applied to this item.");
		return 0;
	}
	...
	if ((GetLargeRand() % 100) < gCustomJewel.GetCustomJewelSuccessRate(lpObj->Inventory[SourceSlot].m_Index, lpObj->AccountLevel))
	{
		...
		if (lpInfo->SetLevel >= 0 && lpItem->m_Level < lpInfo->SetLevel)
		{
			lpItem->m_Level = ((lpInfo->SetLevel > 15) ? 15 : lpInfo->SetLevel);
		}
		else if (lpInfo->Level > 0)
		{
...
bool CObjectManager::CharacterUseWarehouseExpansion(LPOBJ lpObj, CItem* lpItem) // OK
{
	...
	GCMessagePopupSend(lpObj, "Your extra vault has been successfully created. You need to log in again. Press ESC, then Select Server.");
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	// >>> Greater Life BK
	int effectGL = gSkillManager.GetSkillEffect(SKILL_GREATER_LIFE);
	CEffect* pEffGL = gEffectManager.GetEffect(lpObj, effectGL);

	if (pEffGL != nullptr)
	{
		int percent = pEffGL->m_value[0];
		int addLife = (lpObj->MaxLife * percent) / 100;
	...
	if (lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if (lpTarget->Type == OBJECT_USER)
		{
			...
			if (lpObj->Type == OBJECT_USER)
			{
				...
				gNotice.SendMapServerGroupMsg(3, "%s killed %s (%d%% dmg)", gObj[topDamageUser].Name, lpTarget->Name, topDamagePerc);
				...
				if (lpTarget->Map == MAP_CRYWOLF) // && lpTarget->X >= 2 && lpTarget->X <= 62 && lpTarget->Y >= 2 && lpTarget->Y <= 62)
				{
					...
					if (lpTarget->Ruud >= gServerInfo.m_PKRuudDrop) //ROUBA RUUD DO COLEGUINHA
					{
						...
						gNotice.GCNoticeSend(lpTarget->Index, 3, 0 , 0, 0, 0, 0, "You lost %d Ruud to %s", gServerInfo.m_PKRuudDrop, gObj[topDamageUser].Name);
						...
						gNotice.GCNoticeSend(topDamageUser, 1, 0, 0, 0, 0, 0, "You stole %d Ruud from %s", (gServerInfo.m_PKRuudDrop - gServerInfo.m_PKRuudTax), lpTarget->Name);

						gNotice.SendMapServerGroupMsg(1, "%s stole %d Ruud from %s", gObj[topDamageUser].Name, gServerInfo.m_PKRuudDrop, lpTarget->Name);
					}
					else
					{
		...
		if (lpObj->isBoss)
		{
			...
			{
			case 0:
				gNpcTalk.SendNPCMessageToViewPort(lpObj, "GG, you died, your problem! hahaha%s", lpTarget->Name);
				break;
			case 1:
				gNpcTalk.SendNPCMessageToViewPort(lpObj, "hahaha FLY %s TRASH!", lpTarget->Name);
				break;
			case 2:
				gNpcTalk.SendNPCMessageToViewPort(lpObj, "Oof, call the coroner for %s hahaha", lpTarget->Name);
				break;
			case 3:
				gNpcTalk.SendNPCMessageToViewPort(lpObj, "Is there bread in heaven? And he died… hahaha %s", lpTarget->Name);
				break;
			}
		}
