...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	switch(lpNpc->Class)
	{
		...
		case 375:
			...
			break;
		case 407:
			this->NpcWerewolfQuarel(lpNpc, lpObj);
			break;
		case 408:
			this->NpcGatekeeper(lpNpc, lpObj);
			break;
		case 450:
...
void CNpcTalk::NpcWerewolfQuarel(LPOBJ lpNpc, LPOBJ lpObj) // OK
{
	const int RequiredZen = 3000000;

	// doar din Crywolf are sens
	if (lpObj->Map != MAP_CRYWOLF)
	{
		return;
	}

	if (lpObj->Money < RequiredZen)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
			"You need %d Zen to move to Barracks.", RequiredZen);
		return;
	}

	lpObj->Money -= RequiredZen;
	GCMoneySend(lpObj->Index, lpObj->Money);

	// TODO: ajusteaza coordonatele cum vrei tu
	gObjTeleport(lpObj->Index, MAP_BARRACKS, 30, 79);
}

void CNpcTalk::NpcGatekeeper(LPOBJ lpNpc, LPOBJ lpObj) // OK
{
	GCMoneySend(lpObj->Index, lpObj->Money);

	// TODO: ajusteaza coordonatele cum vrei tu
	gObjTeleport(lpObj->Index, MAP_REFUGE, 30, 79);// TO DO change coords
}
