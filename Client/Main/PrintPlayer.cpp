...
DWORD LevelUpPoints = 0;

void InitPrintPlayer()
{
	SetCompleteHook(0xE9, MAIN_OFFSET_PRINT_PLAYER_VIEW_POINTS_HOOK, &PrintPlayerLevelUpPoints);
...
__declspec(naked) void PrintPlayerLevelUpPoints()
{
	static DWORD PrintPlayerLevelUpPointsAddress1 = MAIN_OFFSET_PRINT_PLAYER_VIEW_POINTS;

	_asm
	{
		Mov Eax, LevelUpPoints
		Jmp [PrintPlayerLevelUpPointsAddress1]
	}
}