...
{
	//LogAdd(LOG_DEBUG, "%s %d %d", __FUNCTION__, __LINE__, lpObj->LoadQuestWorld);
...
void CQuestWorld::DGQuestWorldRecv(SDHP_QUEST_WORLD_RECV* lpMsg) // OK
{
	...
	{
		//LogAdd(LOG_DEBUG, "return quest load já carregado");
		return;
	}

	//LogAdd(LOG_DEBUG, "quest do load carregado");
...
void CQuestWorld::GDQuestWorldSend(int aIndex) // OK
{
	...
	{
		//LogAdd(LOG_DEBUG, "return quest load não carregado");
		return;
	}
