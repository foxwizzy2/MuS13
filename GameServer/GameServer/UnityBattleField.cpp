...
//		lpObj->AccountID, lpObj->Name, result);
//
//	g_Log.Add("[UBF][DGAnsCopyCharcterInfo][Copy Complete][%s][%s] CharInfoSave : Class=%d TimerRandom=%d LVPoint=%d Exp=%I64d Str=%d Dex=%d Vit=%d Energy=%d Leadership:%d Map=%d Pk=%d",
//		lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->DbClass, lpObj->TimerRandom, lpObj->m_PlayerData->LevelUpPoint, lpObj->m_PlayerData->Experience, lpObj->m_PlayerData->Strength, lpObj->m_PlayerData->Dexterity,
