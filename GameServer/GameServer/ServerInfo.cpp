...
void CServerInfo::ReadCommonInfo() // OK
{
	...
	gEffectManager.Load(gPath.GetRightPath("..\\Data\\Effect.txt"));
...
void CServerInfo::ReadCustomInfo() // OK
{
	...
	//new
	gMonsterStats.Load(gPath.GetRightPath("Custom\\MonsterStats.xml"));
	gCustomJewelSystem.LoadJewelOfHex(gPath.GetRightPath("Custom\\JewelOfHex.xml"));
...
void CServerInfo::ReloadBC()
{
	char path[260];
	strcpy(path, gPath.GetRightPath("Event\\BloodCastle.dat"));
	if (GetFileAttributesA(path) == INVALID_FILE_ATTRIBUTES) {
		LogAdd(LOG_RED, "[ServerInfo] BloodCastle.dat NOT FOUND!");
		return;
	}
	gBloodCastle.Load(path);
	LogAdd(LOG_BLUE, "[ServerInfo] Blood Castle loaded successfully");
}

void CServerInfo::ReloadDS()
{
	char path[260];
	strcpy(path, gPath.GetRightPath("Event\\DevilSquare.dat"));
	if (GetFileAttributesA(path) == INVALID_FILE_ATTRIBUTES) {
		LogAdd(LOG_RED, "[ServerInfo] DevilSquare.dat NOT FOUND!");
		return;
	}
	gBloodCastle.Load(path);
	LogAdd(LOG_BLUE, "[ServerInfo] Devil Square loaded successfully");
}

void CServerInfo::ReloadCC()
{
	char path[260];
	strcpy(path, gPath.GetRightPath("Event\\ChaosCastle.dat"));

	if (GetFileAttributesA(path) == INVALID_FILE_ATTRIBUTES) {
		LogAdd(LOG_RED, "[ServerInfo] ChaosCastle.dat NOT FOUND!");
		return;
	}

	gChaosCastle.Load(path);
	LogAdd(LOG_BLUE, "[ServerInfo] Chaos Castle loaded successfully");
}
...
//new
void CServerInfo::ReloadGoldenInvasion()
{
	gInvasionManager.Load(gPath.GetRightPath("Event\\InvasionManager.dat"));
}

void CServerInfo::ReloadMonsterStats()
{
	gMonsterStats.Reload();
}
//=====================================================================================
