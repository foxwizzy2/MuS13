...
BOOL gObjMoveGate(int aIndex,int gate) // OK
{
	...
	if (gGate.GetInfo(gate, &info) == 1)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "This location is accessible only for VIP accounts");
			goto ERROR_JUMP;
		}
	}
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if(lpObj->Type == OBJECT_NPC)
			{
				...
				{
					gNpcTalk.SendNPCMessageToViewPort(lpObj, "Don’t forget that each map has a different vault, champ! Your item is in the map where you left it.");
				}
				else if (lpObj->Class == 251)
				{
					gNpcTalk.SendNPCMessageToViewPort(lpObj, "Welcome to ItzyMU’s Item Workshop <3");
				}
			}
			...
			if(lpObj->Type == OBJECT_USER)
			{
				...
				if (lpObj->Map == MAP_CRYWOLF)
				{
					...
					if (lpObj->Ruud < gServerInfo.m_PKRuudDrop)
					{
						gNotice.GCNoticeSend(lpObj->Index, 3, 0, 0, 0, 0, 0, "To stay in Crywolf you need at least %d Ruud", gServerInfo.m_PKRuudDrop);
						gObjMoveGate(lpObj->Index, 17);
					}
				}
