...
int CAttack::GetAttackDamage(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, WORD* effect, int TargetDefense) // OK
{
	...
	// >>> ADD: GREATER DAMAGE EFFECT <<<
	int effectGD = gSkillManager.GetSkillEffect(SKILL_GREATER_DAMAGE);
	CEffect* pEffGD = gEffectManager.GetEffect(lpObj, effectGD);

	if (pEffGD != nullptr)
	{
		int addDmg = pEffGD->m_value[0]; // value1

		lpObj->PhysiDamageMinRight += addDmg;
		lpObj->PhysiDamageMaxRight += addDmg;
		lpObj->PhysiDamageMinLeft += addDmg;
		lpObj->PhysiDamageMaxLeft += addDmg;
	}	// <<< END >>>
