...
void CPersonalShop::CGPShopOpenRecv(PMSG_PSHOP_OPEN_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCMessagePopupSend(lpObj, "You can’t open your personal store without authenticating your PIN first. Use the /pin command followed by your PIN to do it right now.");
		return;
	}
...
void CPersonalShop::CGPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->PersonalCodeChecked == false)
	{
		GCMessagePopupSend(lpObj, "You can’t buy items from a personal store without authenticating your PIN first. Use the /pin command followed by your PIN to do it right now.");
		return;
	}
