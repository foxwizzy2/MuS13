...
bool gObjSetMonster(int aIndex, int MonsterClass, bool DeleteAfterDie, bool RespawnAfterDie, int Element) // OK
{
	...
	gMonsterStats.Apply(
		...
		lpObj->Life,
		lpObj->MaxLife,
		lpObj->PhysiDamageMin,
		lpObj->PhysiDamageMax,
		lpObj->Defense,
		lpInfo->AttackRate,
		lpInfo->DefenseRate
	);
...
bool gObjMonsterMoveCheck(LPOBJ lpObj, int tx, int ty) // OK
{
	...
	// custom mobs (PosNum = -2) -> allowed to move freely
	if (lpObj->PosNum == -2)
	{
		return 1;
	}
...
void gObjMonsterDie(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	gInvasionManager.MonsterDieProc(lpObj, lpTarget);
}
...
void CInvasionManager::StartInvasion(int type)
{
	if (type < 0 || type >= MAX_INVASION_TYPE)
		return;

	auto& vec = this->m_InvasionSpawnInfo[type];
	if (vec.empty())
		return;


	LogAdd(LOG_BLACK, "[InvasionManager] Start invasion type %d", type);

	for (int i = 0; i < (int)vec.size(); i++)
	{
		INVASION_SPAWN_INFO& sp = vec[i];

		for (int c = 0; c < sp.Count; c++)
		{
			int index = gObjAddMonster(sp.Map);
			if (index < 0)
				continue;
			...
			int x = (sp.X1 <= sp.X2) ? (sp.X1 + (GetLargeRand() % (sp.X2 - sp.X1 + 1))) : sp.X1;
			int y = (sp.Y1 <= sp.Y2) ? (sp.Y1 + (GetLargeRand() % (sp.Y2 - sp.Y1 + 1))) : sp.Y1;

			lpObj->X = lpObj->TX = lpObj->OldX = lpObj->StartX = lpObj->SpawnX = (BYTE)x;
			lpObj->Y = lpObj->TY = lpObj->OldY = lpObj->StartY = lpObj->SpawnY = (BYTE)y;
			int dx = abs(sp.X2 - sp.X1);
			int dy = abs(sp.Y2 - sp.Y1);
			lpObj->MoveRange = max(dx, dy);
			lpObj->MoveSpeed = 5;

			gObjSetMonster(index, sp.Class, 1, 0, -1);

			LogAdd(LOG_BLACK, "[Invasion %d] Spawn class %d at (%d,%d) map %d",
				type, sp.Class, lpObj->X, lpObj->Y, lpObj->Map);
		}
	}
	if (this->m_InvasionMessage[type].StartMsg[0] != 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0,
			this->m_InvasionMessage[type].StartMsg);
	}
}
